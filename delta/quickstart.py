...
target_delta_table = "./tmp/delta-table"
...
def get_latest_version_from_delta_table(spark: pyspark.sql.SparkSession, target_delta_table: str, debug: bool = False):
    try:
...
def insert_new_data_into_delta_table(df_new_data: pyspark.sql.DataFrame, spark: pyspark.sql.SparkSession,
                                     target_delta_table: str, write_mode: str = "append"):
    ...
    df_with_version.repartition(1).write.mode(write_mode).partitionBy(
        "version").format("delta").save(target_delta_table)


def read_latest_snapshot(spark: pyspark.sql.SparkSession, target_delta_table: str) -> pyspark.sql.DataFrame:
    # deltaTable = DeltaTable.forPath(spark, target_delta_table)
    ...
    max_watermark = spark.read.format("delta").load(target_delta_table).agg(functions.max(filter_column)).collect()[0][
        0]

    return spark.read.format("delta").load(target_delta_table).where(
        functions.col(filter_column) == functions.lit(max_watermark))
...
def read_all_data(spark: pyspark.sql.SparkSession, target_delta_table: str) -> pyspark.sql.DataFrame:
